...
document.getElementById("subir").onclick = () => ajustar(50);
document.getElementById("bajar").onclick = () => ajustar(-50);
...
function evaluar(hand, mesa) {
    ...
    if (trio && pares >= 1) return { valor: 7, nombre: "Full House" };
    if (color) return { valor: 6, nombre: "Color" };
    if (escalera) return { valor: 5, nombre: "Escalera" };
    if (trio) return { valor: 4, nombre: "Trío" };
    if (pares >= 2) return { valor: 3, nombre: "Doble Pareja" };
    if (pares === 1) return { valor: 2, nombre: "Pareja" };
